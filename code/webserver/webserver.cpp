...
WebServer::WebServer(int port, int trigMode, int timeoutMS, bool OptLinger,
            ...
            bool openLog, int logLevel, int logQueSize):timer_(new HeapTimer()),threadpool_(new ThreadPool(threadNum))
            , epoller_(new Epoller()){
                ...
                //初始化操作
                //连接池单例的初始化
                SqlConnPool::Instance()->Init("192.168.2.25",sqlPort,sqlUser,sqlPwd,dbName,connPoolNum);
                //初始化事件，初始化socket
...
bool WebServer::InitSocket_(){
...

    {
        ...
        ret=setsockopt(listenFd_,SOL_SOCKET,SO_LINGER,&optLinger,sizeof(optLinger));//设置影响套接字的选项
        if(ret<0){
            close(listenFd_);
            LOG_ERROR("Init lionger error!!",port_);
            return false;
        }
    }
    int optval=1;
